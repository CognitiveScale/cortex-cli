...
:deploy
    ...
    cmd /c cortex actions deploy action.json --project %PROJECT_NAME% --docker %DOCKER_IMAGE_URL%
...
:tests
    ...
    cmd /c cortex skills invoke --params-file ./test/payload.json %SKILL_NAME% params --project %PROJECT_NAME%
	GOTO :EOF
