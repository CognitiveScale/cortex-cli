...
module.exports.PromoteEnvironmentCommand = class PromoteEnvironmentCommand {
    ...
    execute(promoteDefinition, options) {
        ...
        environment.promoteEnvironment(profile.token, promObj)
            .then((response) => {
                if (response.success) {
                    printSuccess(`Snapshot \'${promObj.snapshotId}\' successfully promoted to \'${promObj.environmentName}\' environment. Instance \'${response.message.instanceId}\' created.`, options);
                } else {
